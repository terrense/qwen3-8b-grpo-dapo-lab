...
### 23. `loss_scale_factor` 悄悄改掉了有效学习率，差点毁掉整个 Dr.GRPO 臂

Dr.GRPO 那个臂跑完，`entropy` 末值是 **0.106**，其他三臂都是 0.31~0.33。
第一反应是「entropy 崩塌了，Dr.GRPO 有问题」。

但看第一步就不对：**D 在 step 1 的 entropy 就是 0.072**。
四个臂从同一个模型出发、同一个 seed，第一步不可能真差 5 倍。

查出来是两件事叠在一起：

**（1）entropy 的「崩塌」是假的。** `actor/entropy` 也走 `loss_agg_mode` 聚合。
`seq-mean-token-sum-norm` 除的是常数 `loss_scale_factor=8192`，而不是实际长度 |y|≈1900。
所以报出来的 entropy 是真实值的 |y|/8192 ≈ 0.23 倍。

验证：D/A 的 entropy 比值均值 = 0.2375，|y|/8192 均值 = 0.2328，**平均相对误差 3.6%**。
底层 entropy 根本没动。

**（2）但 grad_norm 小 4.3 倍是真的。** loss 本身被同一个常数缩放了，
所以 **D 实际上是「有效学习率只有对照 1/4.3 的 Dr.GRPO」**。

这一臂作为 matched comparison 直接废了 —— 它改的不只是算法，还有学习率。

**怎么来的**：我照搬了 verl 文档里那句「`loss_scale_factor` 设成一个常数，
例如 max response length」。文档本身没错，`seq-mean-token-sum-norm` 的设计就是要用常数除。
但在做**跨算法对比**的时候，这个常数选多大会直接决定有效学习率 ——
选 8192 而实际长度 1900，等于偷偷把 LR 降到 23%。

**修正**：`loss_scale_factor` 设成实际平均长度（1900），让平均意义上和 `token-mean` 尺度一致。

**教训**：换 loss 聚合方式的时候，**先检查它有没有顺带改掉梯度的整体尺度**。
一个简单的自检：看 `grad_norm` 在第一步是不是和对照同一个量级 ——
第一步两个臂的权重完全相同，grad_norm 有系统性差异就说明 loss 尺度被改了，
而不是算法效果。

顺带记一条：**`actor/entropy` 这个指标本身是随 `loss_agg_mode` 变的**，
跨不同聚合方式的臂直接比 entropy 数值是没意义的，得先除掉归一化差异。

---

### 24. 同一个算法跑两次，reward 符号翻转、长度差 65%

这条不算 bug，但它把前面所有「算法 A 比 B 好」的结论全部作废了，所以必须记。

两次 **vanilla GRPO** 的 20-update 运行，配置完全相同，只差训练 prompt 池：

| 指标 | M20_grpo | A_grpo | 差异 |
|---|---|---|---|
| reward 后5均值 | **+0.159** | **−0.097** | **符号翻转** |
| 回答长度 末 | **1430** | **2353** | **+65%** |
| clipfrac | 1.74e-4 | 1.22e-4 | −30% |
| ratio p99 | 0.0558 | 0.0552 | **−1.0%** |
| advantage max | 2.389 | 2.326 | **−2.7%** |

**结果类指标差 65~160%，机制类指标复现到 5% 以内。**

原因不神秘：每个 update 只有 16 道题，题目难度的批间方差极大，20 步总共才见过 320 道题。
而 ratio 分布和 advantage 尺度是算法数学本身决定的，跟抽到哪些题没什么关系。

**后果**：四个臂在 validation 上最大差 0.84 个标准误，
而同一算法两次运行的 reward 能翻符号 —— **那些差异全在噪声里**。

**教训**：**做算法对比之前，先花一次运行的钱测同算法的运行间方差。**
不然你会把噪声当成效应，而且没有任何办法发现。

这条也解释了为什么配对长度分析在两次运行上给出相反结论（一次 0.519 像抛硬币，
一次 0.620 明显有效应）—— 不是分析方法有问题，是底层信号就不可复现。


### 25. 我自己的异常检测器报了两个误报，z=112 的那个只偏离了 4.3%

flight recorder 第一次在真实训练里触发告警，两个都是误报。但正因为它冻结了完整证据包，
十分钟就查清了，所以这次的产出是检测器本身的两个设计缺陷。

**（a）MAD 退化。** rollout 时间步间极其稳定：

    历史 [46.369, 44.357, 44.360, 46.371, 30.344, 46.361, 46.373]
    中位数 46.361，MAD 只有 0.0120

当前值 48.368 **只比中位数高 4.3%**，却被算成 **z = +112.5**。

原因很简单：robust-z 的分母是 1.4826 × MAD。指标越稳定 MAD 越小，
分母塌掉之后任何微小偏离都会炸出巨大的 z。
我原来只处理了 `MAD == 0` 的情况，没处理 `MAD ≈ 0`。

讽刺的是，同一个窗口里有个 30.344（比中位数低 35%，是真正值得看的异常）反而没报。

**（b）对零中心带符号量做 robust-z。** `ppo_kl` 的历史：

    [-1.5e-05, -0.0, -1.8e-05, +2.1e-05, -2.1e-05, -1.5e-05]

它本来就在 0 附近正负翻转（这一点我在 #21 里已经证明过）。
当前值 +2.2e-05 只是又一次翻转，算出 z = 5.0。
而"相对中位数偏离 248%"这种描述，在中位数接近 0 时根本不可解释。

**修法：**
1. 加 `MIN_REL_EFFECT = 0.20` 闸门 —— z 超阈值**且**相对中位数偏离 ≥20% 才告警
2. 把 `kl` 从 ADAPTIVE 规则里删掉，代码里写明理由

**验证**（拿本次真实历史回放）：
- 48.368（偏离 4.3%）→ 已抑制
- 120.0（偏离 159%）→ 仍能抓到，没引入漏报

**教训**：robust-z 这类统计检验默认"数据有一定离散度"。
**在一个几乎不动的指标上，任何异常检测都会变成噪声放大器。**
阈值必须同时包含统计显著性（z）和**实际效应量**（相对偏离）—— 只有前者是不够的。

顺带：这次也验证了 incident bundle 的工作流是有用的 ——
告警时自动冻结了 metric window、系统遥测、日志尾巴、进程快照，
root cause 留 PENDING 等人工填。我按流程调查后把两个 bundle 的
Root cause / Fix / Post-fix evidence 都填上了。
