...
窗口历史：[46.369, 44.357, 44.360, 46.371, 30.344, 46.361, 46.373]
中位数 46.361、**MAD 仅 0.0120**（因为多数值几乎相同）。
当前值 48.368 **只比中位数高 4.3%**，却被算成 z = +112.5。

根因是 MAD 退化：当一个指标步间极其稳定时 MAD 趋近 0，
robust-z 的分母塌掉，任何微小偏离都会产生巨大的 z 值。
原代码只处理了 MAD 恰好等于 0 的情况，没处理 MAD 接近 0。

## Fix

在 incident_detector.py 加 `MIN_REL_EFFECT = 0.20` 闸门：
除了 robust-z 超阈值，还要求相对中位数的偏离达到 20% 才算异常。

## Post-fix evidence

用本次的真实历史回放验证：
  - 48.368（偏离 4.3%）-> 已抑制，不再告警
  - 120.0 （偏离 159%）-> 仍能抓到，没有引入漏报
